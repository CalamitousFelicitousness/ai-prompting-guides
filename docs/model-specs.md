...
| --- | --- | --- | --- | --- | --- | --- |
| anima-2.9b | text-to-image | 812x1216, 1152x1536, and 1536x1536 which the author calls iffy | 28 to 50 | 3.5 to 5 | euler, res-multistep or er_sde, with the sgm-uniform, beta, beta57 or linear-quadratic scheduler; the author's own pick is euler plus sgm-uniform | COMMUNITY derivative, so these values come from its own card rather than from CircleStone. A layer expansion of anima-base from 28 transformer blocks to 40, inserting deep copies of neighbouring blocks at twelve interleaved positions with their output projections zeroed, so it begins functionally identical to base. About 2.9B parameters. Only the new layers are trained as of preview-v1, with roughly 70 percent of the compute spent at 1024px, and the captions carry no score tags. An int8 conversion ships beside the full weights |
| anima-aesthetic | text-to-image | 512x512 through 1536x1536 | 30 to 50 | 4 to 5 | as anima-base | finetuned on high-quality images only, with the quality tags stripped out of the captions; the owner recommends no score_* tags in either the positive or the negative. Ships as v1.0, v1.0b and v1.1, where v1.0b is the aesthetics finetune alone without the style-adjustment and stabilization LoRAs merged into v1.0; the owner prefers v1.0 |
| anima-base | text-to-image | 512x512 through 1536x1536 | 30 to 50 | 4 to 5 | several work; the owner's default is er_sde for flat colors and sharp lines, euler_a gives softer thinner lines and tolerates higher CFG, and dpmpp_2m_sde_gpu is more varied but sometimes too wild | the HF card says CFG 4 to 5 and the CivitAI page says 4 to 6; the beta57 scheduler from the RES4LYF node pack is suggested for painterly texture |
| anima-turbo | text-to-image | 512x512 through 1536x1536 | 8 to 12 | 1 | euler suits it, since the distilled tier is naturally more stable | distilled. At CFG 1 there is no classifier-free guidance, so negative prompts and exclusions have no channel to act through. Distillation also raises stability and gives a strong default style while reducing diversity. The owner recommends it as the starting point, calling it only slightly worse than the aesthetic tier. Ships as v1.0 and v1.1 |
