...
- THE OWNER CONTRADICTS ITSELF HERE, so know which way you are erring. The rewriting spec calls the tags mandatory from two inputs up; the 2.1 repository's own multi-reference quick start passes three images in one untagged sentence. Untagged prompts therefore are not broken, they are the case where every image holds the same role. Tag whenever the images do different jobs, which is when a mix-up would cost you the generation anyway.
